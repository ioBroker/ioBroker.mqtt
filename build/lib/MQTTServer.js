...
class MQTTServer {
    ...
    constructor(adapter, states) {
        ...
        this.config.sendOnStartInterval = parseInt(this.config.sendOnStartInterval, 10) || 2000;
        this.config.sendInterval = parseInt(this.config.sendInterval, 10) || 0;
        this.init();
    }
    destroy(cb) {
        if (this.resendTimer) {
     ...
     */
    mayReceiveOwnMessage(client, ignoreClient, id) {
     ...
     */
    resolveTopicAlias(client, packet) {
    ...
    getMqttMessage(client, id, state, qos, retain, cb, delivery = 'live') {
        if (!this.id2topic[id]) {
            void this.adapter.getForeignObject(id, (err, obj) => {
                ...
                if (!obj._id) {
                    cb(`Client [${client.id}] Cannot resolve topic name for ID: ${id} (object has no id): ${JSON.stringify(obj)}`);
                    return;
                }
                if (!obj.native?.topic) {
                    this.id2topic[obj._id] = (0, common_1.convertID2topic)(obj._id, null, this.config.prefix, this.adapter.namespace, this.config.removePrefix);
                }
                else {
        ...
        if (client._subsID?.[id]) {
            ...
            if (!state) {
                ...
                payload = (0, common_1.state2string)(state.val);
            }
            message = {
    ...
    addMessageAttributes(message, attributes) {
        return {
            ...message,
    ...
    sendMessage2Client(client, topic, payload, retain, binary) {
        const message = this.addMessageAttributes({
            topic,
            payload: (0, common_1.state2string)(payload),
            binary,
            retain,
            ts: Date.now(),
            count: 0,
    ...
    async processTopic(id, topic, message, qos, retain, isAck, ignoreClient, expiryInterval) {
        ...
        if (obj &&
            ...
            try {
                ...
                this.adapter.log.error(`Could not update object "${id}": ${e.message}`);
            }
        }
        if (this.config.debug) {
        ...
        if (parsedForType.message != null) {
            ...
            if (value.val !== undefined &&
                ...
                this.adapter.log.info(`Server: Changed type of "${this.topic2id[topic].id}" from "${this.topic2id[topic].obj.common.type}" to "mixed"`);
                try {
                    await this.adapter.extendObject(this.topic2id[topic].id, { common: { type: 'mixed' } });
                }
                catch (err) {
                    this.adapter.log.warn(`Error while updating object "${this.topic2id[topic].id}" for Server: ${err}`);
                }
                this.topic2id[topic].obj.common.type = 'mixed';
            }
            try {
    ...
    async checkObject(id, topic, message) {
        ...
        this.topic2id[topic] ||= {
            id: '',
            obj: { _id: '', common: {}, native: {}, type: 'state' },
            processing: true,
        };
        if (this.config.debug) {
     ...
     */
    async storeBinaryMessage(id, topic, message, isAck, qos, retain, ignoreClient) {
        ...
        if (!this.topic2id[topic]?.obj?.native?.binary) {
            ...
            (0, common_1.ensureObjectStructure)(this.adapter, fullId, this.verifiedObjects).catch(e => this.adapter.log.error(e));
        }
    ...
    async receivedTopic(packet, client) {
        ...
        if ((0, common_1.isBinaryTopic)(id, this.binaryTopicsRegexes)) {
            await this.storeBinaryMessage(id, topic, message, isAck, qos, retain, client);
            return;
        }
    ...
    validateRequest(type, client) {
        ...
        if (this.clients[client.id] && client.__secret !== this.clients[client.id].__secret) {
            this.adapter.log.debug(`Old client ${client.id} with secret ${client.__secret} sends ${type}. Ignore! Actual secret is ${this.clients[client.id].__secret}`);
            return false;
        }
        return true;
    }
    startServer(socket, server, port, bind, ssl, ws) {
        ...
        socket.on(connectionEvent, (stream) => {
            const client = new MqttConnection_1.default(ws ? (0, websocket_stream_1.default)(stream) : stream);
            ...
            client.on('publish', async (packet) => {
                ...
                if (this.config.debug) {
                    this.adapter.log.debug(`Client [${client.id}] received publish package ${JSON.stringify(packet)}`);
                }
                if (this.persistentSessions[client.id]) {
                    this.persistentSessions[client.id].lastSeen = Date.now();
                }
                if (packet.qos === 1) {
                    ...
                    if (pack) {
                        ...
                        this.adapter.log.info(`Client [${client.id}] Ignored duplicate message with ID: ${packet.messageId}`);
                        return;
                    }
                    packet.ts = Date.now();
                    packet.cmd = 'pubrel';
            ...
            client.on('pubrec', (packet) => {
                ...
                if (this.config.debug) {
                    this.adapter.log.debug(`Client [${client.id}] received pubrec package ${JSON.stringify(packet)}`);
                }
                if (this.persistentSessions[client.id]) {
                    this.persistentSessions[client.id].lastSeen = Date.now();
                }
                ...
                if (frame) {
                    ...
                    try {
                        ...
                        this.adapter.log.warn(`Client [${client.id}] Cannot send pubrel for unknown messageId ${packet.messageId}: ${e}`);
                    }
                }
            });
            // response for QoS2
            ...
            client.on('pubrel', async (packet) => {
                ...
                if (pos !== -1) {
                    ...
                    this.adapter.log.info(`Client [${client.id}] Received pubrel for unknown message ID: ${packet.messageId}`);
                }
            });
            // response for QoS1
            ...
            client.on('subscribe', async (packet) => {
                ...
                if (this.config.debug) {
                    this.adapter.log.debug(`Client [${client.id}] received subscribe package ${JSON.stringify(packet)}`);
                }
                if (this.persistentSessions[client.id]) {
                    this.persistentSessions[client.id].lastSeen = Date.now();
                }
                ...
                for (let i = 0; i < packet.subscriptions.length; i++) {
                    this.adapter.log.debug(`Client [${client.id}] Handle subscribe: ${JSON.stringify(packet.subscriptions[i])}`);
            ...
            client.on('unsubscribe', (packet) => {
                ...
                if (this.config.debug) {
                    this.adapter.log.debug(`Client [${client.id}] received unsubscribe package ${JSON.stringify(packet)}`);
                }
                if (this.persistentSessions[client.id]) {
                    this.persistentSessions[client.id].lastSeen = Date.now();
                }
                    ...
                    if (!id.includes('*') && !id.includes('#') && !id.includes('+')) {
                        ...
                        if (client._subs?.[topic]) {
                            ...
                            if (client._subs[`${this.adapter.namespace}/${topic}`]) {
                                ...
                                this.adapter.log.info(`Client [${client.id}] unsubscribes on "${this.adapter.namespace}/${topic}"`);
                            }
                        }
                        else {
            ...
            client.on('error', (e) => this.clientClose(client, e.toString()));
        ...
        (server || socket)
            .listen(port, bind, () => {
            this.adapter.log.info(`Starting MQTT${ws ? '-WebSocket' : ''}${ssl ? ' (Secure)' : ''}${this.config.user ? ' authenticated' : ''} server on ${bind || '0.0.0.0'}:${port}`);
            this.adapter.log.debug(`The MQTT server${ws ? ' (WebSocket)' : ''} has successfully started!`);
        })
            .on('error', (err) => this.adapter.log.error(`Error starting MQTT (Setting up listener)${ws ? '-WebSocket' : ''}${ssl ? ' (Secure)' : ''}${this.config.user ? ' authenticated' : ''} server on ${bind || '0.0.0.0'}:${port}: ${err}`));
    }
    checkResends() {
        const now = Date.now();
        this.resending = true;
            ...
            if (Object.prototype.hasOwnProperty.call(this.clients, clientId) && this.clients[clientId]?._messages) {
                for (let m = this.clients[clientId]._messages.length - 1; m >= 0; m--) {
                    ...
                    if (now - message.ts >= this.config.retransmitInterval) {
                        ...
                        message.ts = now;
                        try {
                            this.adapter.log.debug(`Client [${clientId}] Resend for ${message.cmd} message topic: ${message.topic}, payload: ${message.payload}`);
    ...
    init() {
        ...
        if (this.config.storeClientsTime === undefined) {
            ...
            this.config.storeClientsTime = parseInt(this.config.storeClientsTime, 10) || 0;
        }
        try {
            ...
            this.server = this.config.ssl ? (0, tls_1.createServer)(this.config.certificates) : (0, net_1.createServer)();
        }
        catch (err) {
        ...
        if (this.config.webSocket) {
            this.serverForWs = this.config.ssl ? (0, https_1.createServer)(this.config.certificates) : (0, http_1.createServer)();
            this.serverWs = new ws_1.Server({ server: this.serverForWs });
            try {
                this.adapter.log.debug(`The MQTT server (Websocket) is starting now...`);
                this.startServer(this.serverWs, this.serverForWs, this.config.port + 1, this.config.bind, this.config.ssl, true);
            }
            catch (err) {
                this.adapter.log.error(`Cannot create server: ${err}`);
                this.adapter.terminate
