...
const node_fs_1 = require("node:fs"); // Load the built-in Node.js file system module
const common_1 = require("./common");
const messageboxRegex = new RegExp('\\.messagebox$');
/**
 * Detects the CONNACK a broker sends when it does not know the requested protocol level:
 * return code 1 in MQTT 3.1/3.1.1, reason code 0x84 in MQTT 5.
 *
 * @param error The error the mqtt client reported
 * @returns Whether the connection failed because of the protocol version
 */
function isUnsupportedVersionError(error) {
    return error?.code === 1 || error?.code === 0x84 || /unacceptable|unsupported protocol version/i.test(`${error}`);
}
const invalidChars = 
...
class MQTTClient {
    ...
    namespaceRegEx;
    connected = false;
    // Protocol level to connect with. Starts at the configured one and is lowered to MQTT 3.1.1
    // once a broker rejects it, so a wrong setting does not leave the adapter in a retry loop.
    protocolVersion;
    protocolVersionRejected = false;
    verifiedObjects = {};
    ...
    patterns = [];
    states;
    constructor(adapter, states) {
        this.adapter = adapter;
        this.states = states;
        this.namespaceRegEx = new RegExp(`^${this.adapter.namespace.replace('.', '\\.')}\\.`);
        this.config = this.adapter.config;
        // The admin UI delivers the version as a string
        const configuredVersion = parseInt(this.config.protocolVersion, 10);
        this.protocolVersion = configuredVersion === 3 || configuredVersion === 5 ? configuredVersion : 4;
        ...
        for (const ignoredTopicPattern of this.ignoredTopics) {
            ...
            const ignoredTopicRegexWithNameSpace = (0, common_1.pattern2RegEx)(`${this.adapter.namespace}.${ignoredTopicPattern}`, this.adapter, this.config.prefix, this.config.dotToUnderscore);
            const ignoredTopicRegex = (0, common_1.pattern2RegEx)(ignoredTopicPattern, this.adapter, this.config.prefix, this.config.dotToUnderscore);
        ...
        for (const binaryTopicPattern of this.binaryTopics) {
            ...
            const binaryTopicRegexWithNameSpace = (0, common_1.pattern2RegEx)(`${this.adapter.namespace}.${binaryTopicPattern}`, this.adapter, this.config.prefix, this.config.dotToUnderscore);
            const binaryTopicRegex = (0, common_1.pattern2RegEx)(binaryTopicPattern, this.adapter, this.config.prefix, this.config.dotToUnderscore);
    ...
    /** Drops the current connection and builds a new one, e.g. with a lowered protocol version. */
    reconnect() {
        if (this.client) {
    ...
    onMessage(topic, message, retain, binary) {
    ...
    send2Server(id, state, cb) {
        ...
        if (state &&
            (0, common_1.isEchoOfReceived)(this.lastReceived[id], JSON.stringify(state.val ?? null), Date.now(), this.config.noEchoInterval)) {
        ...
        if (!topic) {
            void this.adapter.getForeignObject(id, (_err, obj) => {
                ...
                let _topic;
                if (!obj.native?.topic) {
                    _topic = (0, common_1.convertID2topic)(obj._id, null, this.config.prefix, this.adapter.namespace, this.config.removePrefix);
                }
                else {
                    _topic = obj.native.topic;
                }
                this.id2topic[obj._id] = _topic;
                this.topic2id[_topic] ||= { obj: obj, id: obj._id };
    ...
    publishMessage(topic, message, retain, binary) {
        ...
        if (!this.topic2id[topic]) {
            ...
            if (!binary) {
                message = (0, common_1.state2string)(message, this.config.sendStateObject);
            }
        }
        else {
            message =
        ...
        this.client?.publish(topic, message, { qos: this.config.defaultQoS, retain: retain ?? this.config.retain }, err => err && this.adapter.log.error(`client publishMessage: ${err}`));
    }
    /**
     ...
     */
    async storeBinaryMessage(topic, id, message, isAck) {
        const file = (0, common_1.topic2filename)(topic);
        ...
        if (!this.topic2id[topic]?.obj) {
            ...
            if (!obj) {
                ...
                try {
                    ...
                    this.adapter.log.error(`Could not create object "${fullId}": ${err.message}`);
                    return;
                }
            }
            else if (!obj.native?.binary ||
                ...
                try {
                    ...
                    this.adapter.log.error(`Could not update object "${fullId}": ${err.message}`);
                }
            }
            this.topic2id[topic] = { id: fullId, obj: obj };
            this.id2topic[fullId] = topic;
            (0, common_1.ensureObjectStructure)(this.adapter, fullId, this.verifiedObjects).catch(e => this.adapter.log.error(`Cannot ensure object structure: ${e}`));
        }
        ...
        try {
            ...
            this.adapter.log.error(`Cannot store binary payload of "${topic}" as file "${file}": ${err}`);
            return;
        }
        ...
        try {
            ...
            this.adapter.log.warn(`Error while setting binary state "${entry.id}" for Client: ${err}`);
        }
    }
    /**
     ...
     */
    async publishBinaryFromFile(topic, retain) {
        const file = this.topic2id[topic]?.obj?.native?.file;
        if (!file) {
            return;
        }
        try {
            ...
            this.client?.publish(topic, data, { qos: this.config.defaultQoS, retain: retain ?? this.config.retain }, err => err && this.adapter.log.error(`client publishBinary: ${err}`));
        }
        catch (err) {
            this.adapter.log.warn(`Cannot read binary file "${file}" for topic "${topic}": ${err}`);
        }
    }
    publishAllStates(toPublish) {
        if (!toPublish?.length) {
        ...
        if (this.topic2id[this.id2topic[id]]?.obj?.native?.binary) {
            void this.publishBinaryFromFile(this.id2topic[id]).finally(() => setImmediate(() => this.publishAllStates(toPublish)));
            return;
        }
        if (this.config.extraSet && this.states[id] && !this.states[id].ack) {
            this.client.publish(`${this.id2topic[id]}/set`, (0, common_1.state2string)(this.states[id], this.config.sendStateObject), { qos: this.config.defaultQoS, retain: this.config.retain }, err => {
                if (err) {
            ...
            if (this.states[id]) {
                this.client.publish(this.id2topic[id], (0, common_1.state2string)(this.states[id], this.config.sendStateObject), { qos: this.config.defaultQoS, retain: this.config.retain }, err => {
    ...
    init() {
        ...
        if (this.config.ssl && this.config.selfSigned) {
            if (this.config.rootCertificatePath) {
                ...
                this.adapter.log.debug(`SSL and self-signed certificates are enabled. Using root certificate at: ${rootCertPath}`);
            }
            else {
                this.adapter.log.warn(`SSL and self-signed certificates are enabled, but the root certificate path was not set in the UI.`);
            }
        }
        else {
            this.adapter.log.debug(`SSL and/or self-signed certificates not enabled. Root certificate will not be used.`);
        }
        try {
            const opts = {
                clientId: clientId,
                keepalive: this.config.keepalive || 10 /* in seconds */,
                // MQTT 3.1 uses the older "MQIsdp" protocol name
                protocolId: this.protocolVersion === 3 ? 'MQIsdp' : 'MQTT',
                ...
                rejectUnauthorized: this.config.rejectUnauthorized /* added option to disable certification validation */,
            };
            // If SSL and self-signed are used, try to load the certificate!
            if (rootCertPath) {
                try {
                    opts.ca = (0, node_fs_1.readFileSync)(rootCertPath);
                }
                catch (err) {
                    this.adapter.log.error(`Could not read root certificate from: ${rootCertPath}. Error: ${err.message}`);
                }
            }
            if (this.config.user) {
            ...
            this.client = (0, mqtt_1.connect)(_url, opts);
        }
        catch (err) {
            this.adapter.log.error(`Can not connect to mqtt server: ${err}`);
        ...
        this.adapter.getObject('info.connection', (_err, obj) => {
            if (!obj || !obj.common || obj.common.type !== 'boolean') {
                ...
                this.adapter.setObject('info.connection', obj, () => this.adapter.setState('info.connection', this.connected, true));
            }
        });
        ...
        if (this.config.persistent) {
            this.adapter.getObject('info.lastSession', (_err, obj) => {
                if (!obj || !obj.common || obj.common.type !== 'string') {
                    ...
                    this.adapter.setObject('info.lastSession', obj, () => this.adapter.setState('info.lastSession', '', true));
                }
            });
        }
        ...
        this.client.on('message', async (topic, message) => {
            ...
            if (id.length > this.config.maxTopicLength) {
                this.adapter.log.warn(`[${this.client ? this.config.clientId : 'disconnected'}] Topic name is too long: ${id.substring(0, 100)}...`);
                return;
            }
            if (id.match(invalidChars)) {
                this.adapter.log.warn(`[${this.client ? this.config.clientId : 'disconnected'}] Topic name contains invalid characters: ${id} ... ignoring`);
                return;
            }
            ...
            if ((0, common_1.isBinaryTopic)(id, this.binaryTopicsRegexes)) {
            ...
            if (!this.topic2id[topic]) {
                ...
                let stateType = Array.isArray(parsedMessage.message)
                    ? 'array'
                    : messageType === 'string' ||
                        messageType === 'number' ||
                        messageType === 'boolean' ||
                ...
                if (typeWasUndefined) {
                    ...
                    // expand an old version of objects
                    if (this.namespaceRegEx.test(stateObj._id) &&
                        ...
                        if (stateObj.common && stateObj.common.type !== 'mixed' && stateObj.common.type !== stateType) {
                            ...
                            this.adapter.log.info(`Client: Changed type of "${stateObj._id}" from "${stateObj.common.type}"(${message.toString()}) to "${stateType}"(${JSON.stringify(parsedMessage.message)})`);
                            stateObj.common.type = stateType;
                        }
                        stateObj.native ||= {};
                        stateObj.native.topic = topic;
                        try {
                            ...
                            this.adapter.log.error(`Could not update object "${id}": ${err.message}`);
                        }
                    }
                }
                ...
                if (this.config.debug) {
                    this.adapter.log.debug(`Client received "${topic}" (${typeof this.topic2id[topic].message?.message}): ${JSON.stringify(this.topic2id[topic].message)}`);
                }
                try {
                    if (typeof parsedMessage.message === 'object') {
                        ...
                        if (!parsedMessage.isStateObject) {
                            ...
                            await this.adapter.setForeignStateAsync(stateObj._id, JSON.stringify(parsedMessage.message));
                        }
                    }
                    else {
                        await this.adapter.setForeignStateAsync(stateObj._id, {
                    ...
                    this.adapter.log.warn(`Error while setting state "${this.topic2id[topic].id}" for Client: ${err}`);
                }
                // Remember the received value so it is not published straight back (loop protection, #414)
                this.rememberReceived(stateObj._id, parsedMessage.isStateObject
                ...
                (0, common_1.ensureObjectStructure)(this.adapter, stateObj._id, this.verifiedObjects).catch(e => this.adapter.log.error(`Cannot ensure object structure: ${e}`));
            }
                ...
                if (typeof parsedMessage.message === 'object') {
                    if (!this.config.onchange ||
                        ...
                        if (this.config.debug) {
                            this.adapter.log.debug(`Client received "${topic}" (${typeof parsedMessage.message}): ${JSON.stringify(parsedMessage.message)}`);
                        }
                        value = parsedMessage.message;
                    }
                    else if (this.config.debug) {
                        this.adapter.log.debug(`Client received (but ignored) "${topic}" (${typeof parsedMessage.message}): ${JSON.stringify(parsedMessage.message)}`);
                    }
                }
                else {
                    ...
                    if (!this.config.onchange ||
                        ...
                        if (this.config.debug) {
                            this.adapter.log.debug(`Client received "${topic}" (${typeof parsedMessage.message}): ${parsedMessage.message}`);
                        }
                        value = { val: parsedMessage.message, ack: isAck };
                    }
                    else if (this.config.debug) {
                        this.adapter.log.debug(`Client received (but ignored) "${topic}" (${typeof this.topic2id[topic].message?.message}): ${storedMessage.message}`);
                    }
                }
                ...
                if (value !== undefined &&
                    ...
                    stateObj.common.type !== 'mixed' &&
                    stateObj.common.type !== stateType) {
                    this.adapter.log.info(`Client: Changed type of "${this.topic2id[topic].id}" from "${stateObj.common.type}" to "mixed"`);
                    try {
                        ...
                        this.adapter.log.warn(`Error while updating object "${this.topic2id[topic].id}" for Client: ${err}`);
                    }
                    stateObj.common.type = 'mixed';
                }
                if (this.topic2id[topic].id === `${this.adapter.namespace}.info.connection`) {
                    this.adapter.log.debug(`Ignore State update for ${this.topic2id[topic].id} because this.adapter internal state.`);
                }
                else if (value !== undefined && typeof value === 'object' && value.val !== undefined) {
                    try {
                        await this.adapter.setForeignStateAsync(this.topic2id[topic].id, value);
                        // Remember the received value so it is not published straight back (loop protection, #414)
        ...
        this.client.on('connect', () => {
            ...
            if (this.config.publishAllOnStart) {
                ...
                Object.keys(this.states).forEach(id => {
                    if (!messageboxRegex.test(id) && !(0, common_1.isIgnoredTopic)(id, this.ignoredTopicsRegexes)) {
        ...
        this.client.on('error', err => {
            // A broker that does not know the requested protocol level answers with CONNACK
            // "unacceptable protocol version" (MQTT 3.1.1) resp. "unsupported protocol version"
            // (MQTT 5). Retry with MQTT 3.1.1 instead of reconnecting forever.
            if (this.protocolVersion !== 4 && !this.protocolVersionRejected && isUnsupportedVersionError(err)) {
                this.protocolVersionRejected = true;
                this.adapter.log.warn(`The broker does not support MQTT ${this.protocolVersion === 5 ? '5.0' : '3.1'}. Falling back to MQTT 3.1.1.`);
