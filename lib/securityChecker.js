...
function _checkMqttServer(url, protocolVersion) {
    return new Promise((resolve, reject) => {
        const opts = {
            clientId: 'ioBrokerChecker',
            keepalive: 1 /* in seconds */,
            protocolId: 'MQTT',
            protocolVersion: protocolVersion || 4 /* MQTT protocol version: 4 for v3.1.1 (default), 5 for MQTT v5 */,
 ...
 */
async function checkPublicIP(port, secure, protocolVersion) {
    ...
    if (publicIP) {
        ...
        await _checkMqttServer(`${secure ? 'mqtts' : 'mqtt'}://${publicIP}${port ? `:${port}` : ''}`, protocolVersion);

        // check http://publicIP:80
        if (!secure && port !== 1883) {
            await _checkMqttServer(`mqtt://${publicIP}:1883`, protocolVersion);
        }
        if (port !== 8883) {
            await _checkMqttServer(`mqtts://${publicIP}:8883`, protocolVersion);
        }
    }
}
