...
function MQTTClient(adapter, states) {
    ...
    (function _constructor(config) {
        ...
        client = mqtt.connect(_url, {
            ...
            clean:              config.persistent ? false : true
        });
        ...
        client.on('connect', () => {
            ...
            if (config.persistent) {
                adapter.getState('info.lastSession', function (err, state) {
                    let patternsPrevious = JSON.parse(state.val) || [];

                    let patternsDiff = patternsPrevious.filter(x => !config.patterns.includes(x));
