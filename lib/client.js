...
function MQTTClient(adapter, states) {
    ...
    this.onMessage = (topic, message, binary) => publishMessage(topic, message, binary);
    ...
    function send2Server(id, state, cb) {
        ...
        if (!topic) {
            return adapter.getForeignObject(id, (err, obj) => {
                ...
                topic2id[_topic] = topic2id[_topic] || {
                    obj,
                    id: obj._id
                };
    ...
    function publishMessage(topic, message, binary) {
        if (!message) {
            message = null;
        ...
        if (!topic2id[topic]) {
            ...
            message = topic2id[topic].obj.common.type === 'file' ? message : state2string(message, adapter.config.sendStateObject);
        }
        client.publish(
            topic,
            message,
