    ...
    (function _constructor(config) {
        ...
        if (config.ssl && config.selfSigned) {
            if (config.rootCertificatePath) {
                ...
                adapter.log.debug(`SSL and self-signed certificate are enabled. Using root certifcate at: ${rootCertPath}`);
            } else {
                adapter.log.warn(`SSL and self-signed certificate are enabled, but the root certificate path was not set in he UI.`);
            }
        } else {
            adapter.log.debug(`SSL and/or self-signed certificate not enabled. Root certificate will not be used.`);
        }
                ...
                if (obj.common.type === 'file') {
                    ...
                    try {
                        if (typeof topic2id[topic].message === 'object') {
                            ...
                            const forbiddenProperties = Object.keys(topic2id[topic].message).filter(k => !validProperties.includes(k));
                            if (forbiddenProperties.length) {
                                config.debug && adapter.log.debug(`The message object contains forbidden state properties ${forbiddenProperties.join(', ')}: Saving as JSON`);
