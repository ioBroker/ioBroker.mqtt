...
function MQTTClient(adapter, states) {
    ...
    function publishMessage(topic, message) {
        ...
        client.publish(topic, state2string(message, adapter.config.sendStateObject), {qos: adapter.config.defaultQoS, retain: adapter.config.retain}, err => err && adapter.log.error(`client publishMessage: ${err}`));
    }
        ...
        if (!id2topic[id]) {
            adapter.getForeignObject(id, (err, obj) => {
                ...
                if (!obj) {
                    ...
                    id2topic[obj._id] = convertID2topic(obj._id, null, config.prefix, adapter.namespace, adapter.config.removePrefix);
                } else {
                    id2topic[obj._id] = obj.native.topic;
                }
    ...
    (function _constructor(config) {
        ...
        const _url  = `${(!config.ssl) ? 'mqtt' : 'mqtts'}://${config.url}${config.port ? (':' + config.port) : ''}`;
        const __url = `${(!config.ssl) ? 'mqtt' : 'mqtts'}://${config.url}${config.port ? (':' + config.port) : ''} with clientId=${encodeURIComponent(clientId)}${config.user ? `and credentials ${encodeURIComponent(config.user)}:*******************` : ''}`;
            ...
            if (!topic2id[topic]) {
                ...
                if (!obj) {
                    ...
                    if (!obj) {
                        ...
                        try {
                            await adapter.setForeignObjectAsync(topic2id[topic].id, obj);
                        } catch (err) {
                    ...
                    if (namespaceRegEx.test(obj._id) && ((!obj.native || !obj.native.topic) ||
                        ...
                        if (obj.common && obj.common.type !== 'mixed' && obj.common.type !== messageType) {
                            ...
                            adapter.log.info(`Client: Changed type of "${obj._id}" from "${obj.common.type}" to "${messageType}"`);
                            obj.common.type = messageType;
                        }
                        obj.native       = obj.native || {};
                        obj.native.topic = topic;
                        await adapter.setForeignObjectAsync(obj._id, obj, err => err && adapter.log.error(`Could not update object "${id}": ${err.message}`));
                    }
                ...
                try {
                    if (typeof topic2id[topic].message === 'object') {
                        await adapter.setForeignStateAsync(topic2id[topic].id, topic2id[topic].message, err => err && adapter.log.warn(`Error while setting state "${topic2id[topic].id}" for Client: ${err}`));
                    } else {
                ...
                if (value !== undefined) {
                    if (value.val !== undefined && value.val !== null && namespaceRegEx.test(topic2id[topic].id) && topic2id[topic].obj && topic2id[topic].obj.common && topic2id[topic].obj.common.type !== 'mixed' && topic2id[topic].obj.common.type !== typeof value.val) {
                        ...
                        await adapter.extendForeignObjectAsync(topic2id[topic].id, {common: {type: 'mixed'}}, err =>
                            err && adapter.log.warn(`Error while updating object "${topic2id[topic].id}" for Client: ${err}`));
                    ...
                    if (topic2id[topic].id === adapter.namespace + '.info.connection') {
                        ...
                        await adapter.setForeignStateAsync(topic2id[topic].id, value, err => err && adapter.log.warn(`Error while setting state "${topic2id[topic].id}" for Client: ${err}`));
                    }
                }
            }
        });
