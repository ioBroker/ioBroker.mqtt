...
function MQTTClient(adapter, states) {
    ...
    (function _constructor(config) {
        ...
        const _url  = `${(!config.ssl) ? 'mqtt' : 'mqtts'}://${config.user ? `${encodeURIComponent(config.user)}:${encodeURIComponent(config.pass)}@` : ''}${config.url}${config.port ? (':' + config.port) : ''}?clientId=${encodeURIComponent(clientId)}`;
        const __url = `${(!config.ssl) ? 'mqtt' : 'mqtts'}://${config.user ? `${encodeURIComponent(config.user)}:*******************@` : ''}${config.url}${config.port ? (':' + config.port) : ''}?clientId=${encodeURIComponent(clientId)}`;
        ...
        client.on('message', (topic, message) => {
            ...
            if (!topic2id[topic]) {
                topic2id[topic] = {id: null, isAck: isAck, message: message, obj: null};
                ...
                adapter.getObject(id, (err, obj) => {
                    ...
                    if (!obj) {
                        adapter.getForeignObject(id, (err, obj) => {
                            ...
                            if (!obj) {
                                ...
                                adapter.setForeignObject(topic2id[topic].id, obj, (err) => {
                                    ...
                                    ensureObjectStructure(topic2id[topic].id);
                                });
                        ...
                        if (namespaceRegEx.test(obj._id) && ((!obj.native || !obj.native.topic) ||
                            ...
                            obj.native       = obj.native || {};
                            obj.native.topic = topic;
                            adapter.setForeignObject(obj._id, obj, err => err && adapter.log.error(`Could not update object "${id}": ${err.message}`));
                        }
                ...
                if (value !== undefined) {
                    if (value.val !== undefined && value.val !== null && namespaceRegEx.test(topic2id[topic].id) && topic2id[topic].obj && topic2id[topic].obj.common && topic2id[topic].obj.common.type !== 'mixed' && topic2id[topic].obj.common.type !== typeof value.val) {
                        ...
                        adapter.extendObject(topic2id[topic].id, {common: {type: 'mixed'}}, err => err && adapter.log.warn(`Error while updating object "${topic2id[topic].id}" for Client: ${err}`));
                    ...
                    if (topic2id[topic].id === adapter.namespace + '.info.connection') {
                        ...
                        adapter.setForeignState(topic2id[topic].id, value, err => err && adapter.log.warn(`Error while setting state "${topic2id[topic].id}" for Client: ${err}`));
                    }
                }
            }
        });
