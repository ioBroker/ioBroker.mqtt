...
function MQTTClient(adapter, states) {
    ...
    function publishMessage(topic, message) {
        ...
        client.publish(
            topic,
            topic2id[topic].obj.common.type === 'file' ? message : state2string(message, adapter.config.sendStateObject),
    ...
    (function _constructor(config) {
        ...
        function convertMessage(topic, message) {
            ...
            if (typeof message === 'string') {
                ...
                let _val = message.replace(',', '.');

                if (isFinite(_val)) {
                    message = parseFloat(_val).toString();
                }
                if (message === 'true') {
                    message = true;
                }
            ...
            if (!topic2id[topic]) {
                ...
                if (!obj) {
                    ...
                    if (!obj) {
                        ...
                        if (obj && obj._id && obj.type === 'folder' && obj.native && obj.native.autocreated === 'by automatic ensure logic') {
                            // ignore default created object because we now have a more defined one
                            obj = null;
                        }
                    }
                ...
                if (obj.common.type !== 'file') {
                    message = convertMessage(topic, message);
                ...
                if (obj.common.type === 'file') {
                    ...
                    topic2id[topic].message = convertMessage(topic, topic2id[topic].message);
                    try {
