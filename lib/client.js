...
const fs = require('fs'); // Load the built-in Node.js file system module
...
function MQTTClient(adapter, states) {
    ...
    for (const ignoredTopicPattern of ignoredTopics) {
        if (!ignoredTopicPattern) {
            ...
            if (ignoredTopics.length > 1) {
                adapter.log.warn('Ignored topics should not end with an ",".');
            }
            continue;
        }
    ...
    function send2Server(id, state, cb) {
        ...
        publishMessage(adapter.config.extraSet && state && !state.ack ? `${topic}/set` : topic, state || null);
    ...
    function publishMessage(topic, message, retain, binary) {
        ...
        if (!topic2id[topic]) {
            ...
            message =
                topic2id[topic].obj?.common && topic2id[topic].obj.common.type === 'file'
                    ? message
    ...
    (function _constructor(config) {
        ...
        // This variable will hold the root certificate path provided in the adapter UI.
        let rootCertPath = null;

        // Load certificate only if SSL and self-signed certificate are enabled
        if (config.ssl && config.selfSigned) {
            ...
            adapter.log.debug(`SSL and/or self-signed certificates not enabled. Root certificate will not be used.`);
        }
        ...
        try {
            const opts = {
                ...
                protocolId: 'MQTT',
                protocolVersion: config.protocolVersion || 4 /* MQTT protocol version: 4 for v3.1.1 (default), 5 for MQTT v5 */,
            ...
            if (rootCertPath) {
                try {
                    opts.ca = fs.readFileSync(rootCertPath);
                } catch (err) {
                    adapter.log.error(`Could not read root certificate from: ${rootCertPath}. Error: ${err.message}`);
                }
            }
            ...
            adapter.log.debug(`The connection is now being established to URL: ${__url}`);
        ...
        client.on('message', async (topic, message) => {
            ...
            let id = topic2id[topic]?.id || convertTopic2id(topic, false, config.prefix, adapter.namespace);

            if (id.length > config.maxTopicLength) {
                adapter.log.warn(
                    `[${client ? client.id : 'disconnected'}] Topic name is too long: ${id.substring(0, 100)}...`,
                );
                return;
            }
            if (id.match(invalidChars)) {
                adapter.log.warn(
                    `[${client ? client.id : 'disconnected'}] Topic name contains invalid characters: ${id} ... ignoring`,
                );
                return;
            }
            ...
            if (!topic2id[topic]) {
                ...
                if (obj.common?.type && namespaceRegEx.test(obj._id) && !obj.native?.topic) {
                    obj.native ||= {};
                    ...
                    if (obj.common.type === null) {
                        ...
                        if (
                            ...
                            if (obj.common && obj.common.type !== 'mixed' && obj.common.type !== messageType) {
                                ...
                                adapter.log.info(
                                    `Client: Changed type of "${obj._id}" from "${obj.common.type}"(${originalMessage}) to "${messageType}"(${message})`,
                                );
                                obj.common.type = messageType;
                            }
                            obj.native ||= {};
                ...
                config.debug &&
                    adapter.log.debug(
                        `Client received "${topic}" (${typeof topic2id[topic].message}): ${topic2id[topic].message}`,
                    );
                ...
                if (obj.common.type === 'file') {
                    ...
                    try {
                        if (typeof topic2id[topic].message === 'object') {
                            ...
                            const forbiddenProperties = Object.keys(topic2id[topic].message).filter(
                            ...
                            if (forbiddenProperties.length) {
                                ...
                                await adapter.setForeignStateAsync(topic2id[topic].id, {
                                    val: JSON.stringify(topic2id[topic].message),
                ...
                config.debug &&
                    adapter.log.debug(
                        `Client received (but in process) "${topic}" (${typeof topic2id[topic].message}): ${topic2id[topic].message}`,
                    );
                ...
                if (topic2id[topic].obj.common.type !== 'file') {
                    ...
                    if (typeof message === 'object') {
                        if (!config.onchange || JSON.stringify(topic2id[topic].message) !== JSON.stringify(message)) {
                            config.debug &&
                                adapter.log.debug(`Client received "${topic}" (${typeof message}): ${message}`);
                            value = message;
                        } else {
                            config.debug &&
                                adapter.log.debug(
                                    `Client received (but ignored) "${topic}" (${typeof message}): ${message}`,
                                );
                        }
                    } else {
                        if (
                            ...
                            config.debug &&
                                adapter.log.debug(
                                    `Client received (but ignored) "${topic}" (${typeof topic2id[topic].message}): ${topic2id[topic].message}`,
                                );
                        }
                    }
                    ...
                    if (
                        ...
                        adapter.log.info(
                            `Client: Changed type of "${topic2id[topic].id}" from "${topic2id[topic].obj.common.type}" to "mixed"`,
                        );
                        try {
                    ...
                    if (topic2id[topic].id === `${adapter.namespace}.info.connection`) {
                        adapter.log.debug(
                            `Ignore State update for ${topic2id[topic].id} because adapter internal state.`,
                        );
        ...
        client.on('connect', () => {
            ...
            if (config.publishAllOnStart) {
                ...
                Object.keys(states).forEach(id => {
                    if (!messageboxRegex.test(id) && !isIgnoredTopic(id, ignoredTopicsRegexes)) {
                        adapter.log.info(`Publishing state ${id}`);
                        toPublish.push(id);
                    }
                });
