...
function MQTTServer(adapter, states) {
    ...
    async function updateClients() {
        ...
        if (!channelChecked) {
            channelChecked = true;
            const channel = await adapter.getObjectAsync('info.clients');
            if (!channel) {
                await adapter.setObjectAsync('info.clients', {
                    type: 'channel',
                    common: {
                        name: 'Clients',
                        role: 'info'
    ...
    function clientClose(client, reason) {
        ...
        try {
            if (clients[client.id] && (client.__secret === clients[client.id].__secret)) {
                ...
                updateClients()
                    .catch(e => adapter.log.error(`Cannot update connection info: ${e}`));
        ...
        socket.on(connectionEvent, stream => {
            ...
            client.on('connect', options => {
                ...
                if (adapter.config.forceCleanSession === 'clean') {
                    ...
                    client.cleanSession = options.cleanSession !== undefined
    ...
    (function _constructor(config) {
        ...
        try {
            ...
            server = config.ssl
                ? require('tls').createServer(config.certificates)
        ...
        try {
            adapter.log.debug(`The MQTT erver is starting now...`);
