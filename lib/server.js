...
function MQTTServer(adapter, states) {
    ...
    function receivedTopic(packet, client, cb) {
        ...
        if (qos === 1) {
            ...
            const pack = qos2queue.find(function (e) {
                if (cb) cb();
        ...
        if (qos2queue.length && (!lastQueueCheck || now - lastQueueCheck > 20000)) {
            lastQueueCheck = now;
            for (let q = qos2queue.length - 1; q >= 0; q--) {
                if (now - qos2queue[q].ts > 10000) {
                    adapter.log.warn('Timeout for PUBREL message with ID: ' + qos2queue[q].id);
                    qos2queue.splice(q, 1);
                }
            }
        }
    ...
    function startServer(config, socket, server, port, bind, ssl, ws) {
        socket.on('connection', stream => {
            ...
            client.on('pubrel', function (packet) {
                ...
                var message = qos2queue.find(function (e) {
                    return e.id === packet.messageId;
                });

                if (message) {
                    var pos = qos2queue.indexOf(message);
                    qos2queue.splice(pos, 1);
                } else {
