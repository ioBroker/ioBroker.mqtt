...
function MQTTServer(adapter, states) {
    ...
    async function processTopic(id, topic, message, qos, retain, isAck, obj, ignoreClient, cb) {
        ...
        if (message !== undefined) {
            ...
            if (value.val !== undefined && value.val !== null && namespaceRegEx.test(topic2id[topic].id) && topic2id[topic].obj && topic2id[topic].obj.common && topic2id[topic].obj.common.type !== 'mixed' && topic2id[topic].obj.common.type !== typeof value.val) {
                ...
                await adapter.extendObjectAsync(topic2id[topic].id, {common: { type: 'mixed'}}, err => err && adapter.log.warn(`Error while updating object "${topic2id[topic].id}" for Server: ${err}`));
                topic2id[topic].obj.common.type = 'mixed';
            }
            await adapter.setForeignStateAsync(id, value, (err, id) => states[id] = value);
        } else {
            ...
            if (!obj) {
                adapter.getForeignObject(id, async (err, obj) => {
                    ...
                    if (!obj && adapter.config.ignoreNewObjects) {
                        ...
                        return callback && callback('Object not exists');
                    }
                    if (!obj) {
                        ...
                        try {
                            ...
                            adapter.log.error('setForeignObject: ' + err);
                            callback && callback(err, id, obj);
                        }
                    } else {
    ...
    function receivedTopic(packet, client, cb) {
        ...
        if (!topic2id[topic]) {
            checkObject(id, topic, async (err, id, obj) => {
                if (!err) {
                    await processTopic(id, topic, message, qos, retain, isAck, obj, client);
                }
                cb && cb();
                cb = null;
            });
