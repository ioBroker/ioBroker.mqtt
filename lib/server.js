...
function MQTTServer(adapter, states) {
        ...
        if (adapter.config.onchange && server && message !== undefined) {
            setImmediate(() => {
                ...
                for (const k in clients) {
                    ...
                    if (!clients.hasOwnProperty(k) || (clients[k] === ignoreClient && !adapter.config.extraSet)) {
                        continue;
                    }
    ...
    function receivedTopic(packet, client, isLastWill, cb) {
        if (typeof isLastWill === 'function') {
            cb = isLastWill;
            isLastWill = false;
        }
        ...
        try {
            if (clients[client.id] && (client.__secret === clients[client.id].__secret)) {
                ...
                if (client._will && reason !== 'disconnected') {
                    receivedTopic(client._will, client, true,() => client.destroy());
                } else {
                    client.destroy();
                }
            } else {
