...
function MQTTServer(adapter, states) {
    ...
    function sendState2Client(client, id, state, cb) {
        ...
        if (!client._subsID ||
            ...
            if (adapter.config.extraSet && state && !state.ack) {
                client.publish({topic: topic + '/set', payload: (state ? state2string(state.val) : null)});
            } else {
    ...
    function processTopic(id, topic, message, isAck, obj, ignoreClient) {
    ...
    function checkObject(id, topic, message, isAck, ignoreClient) {
            ...
            if (!obj) {
                adapter.getForeignObject(id, function (err, obj) {
                    if (!obj) {
                        ...
                        processTopic(id, topic, topic2id[topic].message, isAck, null, ignoreClient);
                    } else {
                        processTopic(obj._id, topic, topic2id[topic].message, isAck, obj, ignoreClient);
                    }
                });
            } else {
                processTopic(obj._id, topic, topic2id[topic].message, isAck, obj, ignoreClient);
    ...
    var _constructor = (function (config) {
        var cltFunction = function (client) {
            ...
            client.on('publish', function (packet) {
                ...
                if (!topic2id[topic]) {
                    checkObject(id, topic, message, isAck, client);
                    ...
                    processTopic(topic2id[topic].id, topic, message, isAck, null, client);
                }
            });
