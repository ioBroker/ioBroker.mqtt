...
function MQTTServer(adapter, states) {
    ...
    function checkObject(id, topic, message) {
        var id = adapter.namespace + '.' + id;
            ...
            if (!obj) {
                adapter.getForeignObject(id, function (err, obj) {
                    if (!obj) {
                        ...
                        if (adapter.config.debug) adapter.log.debug('Server received "' + topic + '" (' + typeof topic2id[topic].message + '): ' + topic2id[topic].message);

                        // write
                        if (topic2id[topic].message !== undefined) {
                            if (typeof topic2id[topic].message === 'object') {
                                adapter.setForeignState(topic2id[topic].id, topic2id[topic].message, function (err, id) {
                                    states[id] = topic2id[topic].message;
                                });
                            } else {
                                adapter.setForeignState(topic2id[topic].id, {val: topic2id[topic].message, ack: true}, function (err, id) {
                                    states[id] = {val: topic2id[topic].message, ack: true};
                                });
                            }
                        } else {
                            adapter.setForeignState(topic2id[topic].id, {val: null, ack: true}, function (err, id) {
    ...
    var _constructor = (function (config) {
        var cltFunction = function (client) {
            ...
            client.on('publish', function (packet) {
                ...
                if (!topic2id[topic]) {
                    checkObject(id, topic, message);
                }else if (topic2id[topic].id === null) {
                    if (adapter.config.debug) adapter.log.debug('Server received (but in process) "' + topic + '" (' + typeof topic2id[topic].message + '): ' + topic2id[topic].message);
                    topic2id[topic].message = packet.topic;
                } else {
                    if (topic2id[topic].message !== undefined) delete topic2id[topic].message;
                    if (typeof packet.topic === 'object') {
                        if (adapter.config.debug) adapter.log.debug('Client [' + client.id + '] publishes "' + topic2id[topic].id + '" (' + typeof message + '): ' +  message);
                        adapter.setForeignState(topic2id[topic].id, packet.topic, function (err, id) {
                            states[id] = packet.topic;
                        });
                ...
                /*if (typeof message == 'string' && message[0] == '{') {
                 ...
                 if (states[topic]) {
                 if (config.debug) adapter.log.debug('Client [' + client.id + '] publishes "' + topic + '": ' + JSON.stringify(message));

                 adapter.setForeignState(topic, message, function (id) {
                 states[id] = message;
                 });
                 } else {
                 if (config.debug) adapter.log.debug('Client [' + client.id + '] publishes "' + adapter.namespace + '.' + topic + '": ' + JSON.stringify(message));

                 adapter.setState(topic, message, function (id) {
                 states[id] = message;
                 });
                 }
                 return;
