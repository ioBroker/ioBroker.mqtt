...
function MQTTServer(adapter, states) {
    ...
    function getMqttMessage(client, id, state, qos, retain, cb) {
        ...
        if (!id2topic[id]) {
            return adapter.getForeignObject(id, (err, obj) => {
                if (err) {
                    return cb(`Cannot resolve topic name for ID: ${id} (err: ${err})`);
                }
                if (!obj) {
                    return cb(`Cannot resolve topic name for ID: ${id} (object not found)`);
                } else if (!obj._id) {
                    return cb(`Cannot resolve topic name for ID: ${id} (object has no id): ${JSON.stringify(obj)}`);
                ...
                getMqttMessage(obj._id, state, qos, retain, cb);
            });
        }
    ...
    function sendState2Client(client, id, state, qos, retain, cb) {
        ...
        getMqttMessage(client, id, state, qos, retain, (err, message) => {
            if (message) {
                if (adapter.config.debug) {
                    adapter.log.debug('Send to client [' + client.id + '] "' + message.topic + '": ' + (message.payload !== null ? message.payload : 'deleted'));
                }
    ...
    function resendMessages2Client(client, messages, i) {
        ...
        if (messages && i < messages.length) {
            try {
                ...
                messages[i].count++;
                adapter.log.debug(`Resend messages on connect to ${client.id}: ${messages[i].topic} = ${messages[i].payload}`);
                if (messages[i].cmd === 'publish') {
                    client.publish(messages[i]);
                }
            } catch (e) {
                adapter.log.warn(`Cannot resend message to client ${client.id}: ${e}`);
            }
    ...
    function receivedTopic(packet, client, cb) {
        ...
        if (!topic2id[topic]) {
            ...
            if (adapter.config.debug) {
                adapter.log.debug('Server received (but in process) "' + topic + '" (' + typeof message + '): ' + message);
            }
            if (cb) {
                cb();
                cb = null;
    ...
    function startServer(config, socket, server, port, bind, ssl, ws) {
        socket.on('connection', stream => {
            ...
            client.on('publish', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    return adapter.log.warn(`Old client ${client.id} sends publish. Ignore!`);
                }
                if (packet.qos === 1) {
            ...
            client.on('pubrec', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    return adapter.log.warn(`Old client ${client.id} sends pubrec. Ignore!`);
                }
                let pos = null;
                ...
                if (pos !== -1) {
                    ...
                    adapter.log.warn(`Received pubrec on ${client.id} for unknown messageId ${packet.messageId}`);
                }
            });
            ...
            client.on('pubcomp', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    return adapter.log.warn(`Old client ${client.id} sends pubcomp. Ignore!`);
                }
                let pos = null;
                // remove this message from queue
                client._messages.forEach((e, i) => {
                ...
                if (pos !== null) {
                    ...
                    adapter.log.warn('Received pubcomp for unknown message ID: ' + packet.messageId);
                }
            });
            ...
            client.on('pubrel', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    return adapter.log.warn(`Old client ${client.id} sends pubrel. Ignore!`);
                }
                let pos = null;
                // remove this message from queue
                client._messages.forEach((e, i) => {
                ...
                if (pos !== -1) {
                    ...
                    adapter.log.warn(`Received pubrel on ${client.id} for unknown messageId ${packet.messageId}`);
                }
            });
            ...
            client.on('puback', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    return adapter.log.warn(`Old client ${client.id} sends puback. Ignore!`);
                }
                // remove this message from queue
                let pos = null;
                // remove this message from queue
                ...
                if (pos !== null) {
                    adapter.log.debug(`Received puback for ${client.id} message ID: ${packet.messageId}`);
                    client._messages.splice(pos, 1);
                } else {
                    adapter.log.warn('Received puback for unknown message ID: ' + packet.messageId);
                }
            });

            client.on('subscribe', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    return adapter.log.warn(`Old client ${client.id} sends subscribe. Ignore!`);
                }
                const granted = [];
            ...
            client.on('pingreq', (/*packet*/) => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    return adapter.log.warn(`Old client ${client.id} sends pingreq. Ignore!`);
                }
                ...
                for (let m = clients[clientId]._messages.length - 1; m >= 0; m--) {
                    ...
                    if (now - message.ts >= adapter.config.retransmitInterval) {
                        if (message.count > adapter.config.retransmitCount) {
                            adapter.log.warn(`Message ${message.messageId} for ${clientId} deleted after ${message.count} retries`);
                        ...
                        message.ts = now;
                        try {
                            adapter.log.debug(`Resend message for ${clientId} topic: ${message.topic}, payload: ${message.payload}`);
                            if (message.cmd === 'publish') {
                                clients[clientId].publish(message);
                            }
                        } catch (e) {
                            adapter.log.warn(`Cannot publish message for ${clientId}: ${e}`);
                        }
    ...
    (function _constructor(config) {
        ...
        adapter.config.retransmitInterval = adapter.config.retransmitInterval || 2000;
        adapter.config.retransmitCount = adapter.config.retransmitCount || 10;
        if (adapter.config.storeClientsTime === undefined) {
            adapter.config.storeClientsTime = 1440;
        } else {
            adapter.config.storeClientsTime = parseInt(adapter.config.storeClientsTime, 10) || 0;
        }
