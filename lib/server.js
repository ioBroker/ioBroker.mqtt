...
function MQTTServer(adapter, states) {
        ...
        if (server) {
            setImmediate(() => {
                if (state && state.binary) {
                    ...
                    adapter.getForeignBinaryStateAsync(id)
                        .then(buffer => {
                        ...
                        .catch(error => {
                            adapter.log.warn('Cannot read binary state: ' + error);
                        });
                } else {
                    Object.keys(clients).forEach(k =>
    ...
    async function checkObject(id, topic, message) {
        ...
        if (!obj) {
            ...
            if (!obj) {
                ...
                message = convertMessage(topic, null, message);
                ...
                obj = {
                    ...
                    common: {
                        ...
                        type:  message !== undefined ? typeof message : 'string'
                    },
                    native: {
                        topic
                    },
                ...
                if (obj.common.type === 'object' && message !== undefined && message.val !== undefined) {
    ...
    function convertMessage(topic, clientID, message) {
        ...
        if (type === 'string' && message[0] === '{') {
            try {
                ...
                adapter.log.error(`Client [${clientID}] Cannot parse topic ${topic}: ${message}`);
            }
        }
    ...
    async function receivedTopic(packet, client) {
        ...
        if (topic2id[topic].obj.common.type !== 'file') {
            message = convertMessage(topic, client.id, message);
        }
