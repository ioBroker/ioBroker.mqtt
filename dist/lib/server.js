...
function MQTTServer(adapter, states) {
    ...
    this.onStateChange = (id, state) => {
        ...
        setImmediate(() => {
            Object.keys(clients).forEach(k => sendState2Client(clients[k], id, state, adapter.config.defaultQoS, !adapter.config.noRetain));
            Object.keys(persistentSessions).forEach(clientId => {
                if (!clients[clientId]) {
                    getMqttMessage(persistentSessions[clientId], id, state, adapter.config.defaultQoS, !adapter.config.noRetain, (err, message, persistentClient) => message && persistentClient && addMessageWithTopicCheck(persistentClient.messages, message));
                }
            });
        });
    };
    async function updateClientObjects() {
        ...
        if (clients) {
            for (const id in clients) {
                ...
                else if (clientObj.native.port !== clients[id].stream.remotePort ||
    ...
    function sendMessage2Client(client, topic, payload, qos, retain, binary) {
        const message = addMessageAttributes({ topic, payload, binary }, { qos: client._subsID?.[topic] ? client._subsID[topic].qos : undefined, retain });
        if (message) {
    ...
    function sendStates2Client(client, list) {
        if (list?.length) {
            const id = list.shift();
            sendState2Client(client, id, states[id], 0, !adapter.config.noRetain, () => setTimeout(() => sendStates2Client(client, list), adapter.config.sendInterval));
        }
        else {
            //return;
        }
    ...
    async function checkObject(id, topic, message) {
        ...
        if (obj?._id?.startsWith(`${adapter.namespace}.`) &&
            obj.type === 'folder' &&
    ...
    function startServer(config, socket, server, port, bind, ssl, ws) {
        ...
        (server || socket)
            ...
            .on('error', err => adapter.log.error(`Error starting MQTT (Setting up listener)${ws ? '-WebSocket' : ''}${ssl ? ' (Secure)' : ''}${config.user ? ' authenticated' : ''} server on ${bind || '0.0.0.0'}:${port}: ${err}`));
    }
