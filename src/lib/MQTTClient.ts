 ...
 */
function isUnsupportedVersionError(error: Error & { code?: number }): boolean {
...
export default class MQTTClient {
    ...
    private protocolVersion: 3 | 4 | 5;
    private protocolVersionRejected = false;
    ...
    constructor(adapter: ioBroker.Adapter, states: Record<string, ioBroker.State>) {
        ...
        const configuredVersion = parseInt(this.config.protocolVersion as unknown as string, 10);
    ...
    private init(): void {
        ...
        this.client.on('error', err => {
            ...
            if (this.protocolVersion !== 4 && !this.protocolVersionRejected && isUnsupportedVersionError(err)) {
                ...
                this.adapter.log.warn(
                    `The broker does not support MQTT ${this.protocolVersion === 5 ? '5.0' : '3.1'}. Falling back to MQTT 3.1.1.`,
                );
